...
The venv folder is named `venv/`, not `.venv/` — `.gitignore` lists both names
defensively, but `venv/` is the one actually used. `SETUP.md` and
`commands to run.md` previously disagreed with this (and with each other);
both have been corrected to match.
